#!/bin/bash
# Generic Linux build script for simple-dnsd
...
create_service() {
    if [ "$SERVICE" = "true" ]; then
        ...
        sudo tee /etc/systemd/system/simple-dnsd.service > /dev/null << EOF
[Unit]
Description=Simple DNS Daemon
...
User=simple-dnsd
Group=simple-dnsd
ExecStart=/usr/local/bin/simple-dnsd --daemon
Restart=always
RestartSec=5
...
EOF
        ...
        sudo useradd --system --no-create-home --shell /bin/false simple-dnsd 2>/dev/null || true
        ...
        print_status "Use 'sudo systemctl start simple-dnsd' to start the service"
    fi
}
...
show_usage() {
    cat << EOF
simple-dnsd - Linux Build Script
