...
# simple-dnsd macOS Build Script
# This script builds the simple-dnsd application for macOS
...
build_project() {
    print_status "Building simple-dnsd..."
...
install_project() {
    if [ "$INSTALL" = "true" ]; then
        print_status "Installing simple-dnsd..."
        cd "$BUILD_DIR"
        
        make install
        
        # Test installation
        if command_exists simple-dnsd; then
...
create_static_package() {
    if [ "$ENABLE_STATIC_LINKING" = "ON" ]; then
        ...
        STATIC_DIR="$DIST_DIR/simple-dnsd-$VERSION-static-macos"
        mkdir -p "$STATIC_DIR"
        
        # Copy binary and files
        cp "$BUILD_DIR/simple-dnsd" "$STATIC_DIR/"
        ...
        # Create tarball
        cd "$DIST_DIR"
        tar -czf "simple-dnsd-$VERSION-static-macos.tar.gz" "simple-dnsd-$VERSION-static-macos"
        rm -rf "simple-dnsd-$VERSION-static-macos"
        
        print_success "Static binary package created: simple-dnsd-$VERSION-static-macos.tar.gz"
    fi
}
...
# Main execution
main() {
    print_status "Starting simple-dnsd build process..."
    ...
    print_success "simple-dnsd build process completed successfully!"
}
