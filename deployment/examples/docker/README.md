# simple-dnsd Docker Deployment

This directory contains Docker deployment examples for simple-dnsd.

## Quick Start

1. **Build the Docker image:**
   ```bash
   docker build -t simple-dnsd:latest .
   ```
...
| Volume | Description |
|--------|-------------|
| `./config:/etc/simple-dnsd:ro` | Configuration files (read-only) |
| `./data:/var/lib/simple-dnsd` | Data directory |
| `./logs:/var/log/simple-dnsd` | Log files |
...
# View logs
docker-compose logs -f simple-dnsd

# Restart service
docker-compose restart simple-dnsd
...
The service uses a custom bridge network (`simple-dnsd-network`) with subnet `172.22.0.0/16`.
...
# Check configuration
docker-compose exec simple-dnsd cat /etc/simple-dnsd/simple-dnsd.conf
```
