...
Version:        ${VERSION}
Release:        1%{?dist}
Summary:        simple-dnsd - A lightweight and secure daemon
License:        Apache-2.0
...
%description
simple-dnsd is a modern, secure implementation of the DNS protocol.
...
if [ "$1" -eq 1 ]; then
    ...
    # Create service user
    if ! id dnsddev &>/dev/null; then
        useradd -r -s /sbin/nologin -d /var/lib/%{name} -c "%{name} service user" dnsddev
    fi
fi
