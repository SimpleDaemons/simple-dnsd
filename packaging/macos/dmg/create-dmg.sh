#!/bin/bash
# Create DMG installer for simple-dnsd
...
VERSION="${1:-1.0.0}"
DMG_NAME="$simple-dnsd-${VERSION}"
DMG_TEMP="dmg-temp"
DMG_FILE="${DMG_NAME}.dmg"
APP_NAME="$simple-dnsd.app"
PKG_NAME="$simple-dnsd.pkg"
...
if [ -f "../assets/icons/$simple-dnsd.icns" ]; then
    # Set custom icon (requires additional tools)
    echo "Custom icon available at ../assets/icons/$simple-dnsd.icns"
fi
